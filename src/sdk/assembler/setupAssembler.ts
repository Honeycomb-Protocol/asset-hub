...
import { Signer, Metaplex, toMetaplexFile } from "@metaplex-foundation/js";
        ...
        block.definitions = await Promise.all(block.definitions.map(async (blockDefinition) => {
            ...
            if (blockDefinition.mintAddress || blockDefinition.collection) {
                ...
                if (!uri && blockDefinition.assetConfig.image) {
                    const buffer = await fs.readFile(path.resolve(process.cwd(), blockDefinition.assetConfig.image));
                    const file = toMetaplexFile(buffer, blockDefinition.assetConfig.image.slice(blockDefinition.assetConfig.image.lastIndexOf('/') + 1));
                ...
                const createAssetCtx = createCreateAssetTransaction(
                    wallet.publicKey,
                    {
                        ...
                        candyGuard: blockDefinition.assetConfig.candyGuard ? new web3.PublicKey(blockDefinition.assetConfig.candyGuard) : null,
                        symbol: blockDefinition.assetConfig.symbol,
                        uri: uri
                    }
                )
            ...
            blockDefinition.address = createBlockDefinitionCtx.blockDefinition.toString()
