...
export const fetchAvailableNfts = async ({
        ...
        .map((x) => {
          return {
            ...
            ...ownedTokenAccounts.find(
              (y) =>
                //@ts-ignore
                y.tokenMint === (x.mintAddress || x.mint.address).toString()
            ),
          };
        })
        .filter((x) => {
          if (x.tokenStandard === TokenStandard.ProgrammableNonFungible) {
            return true;
          }
  ...
  if (args.project.allowedMints) {
    ...
    filteredNfts = [
      ...filteredNfts,
      ...ownedNfts.filter((nft) => allowedMints.includes(nft.tokenMint)),
    ];
  }
