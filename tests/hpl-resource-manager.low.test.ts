...
describe("Resource Manager", () => {
  ...
  describe.skip("Non Funglible Resources ", () => {
    ...
    it("Create Resources", async () => {
      if (data.nonFungible.resources.length > 0) return;
      ...
      for (let i = 0; i < 1; i++) {
        ...
        const ix = createCreateResourceInstruction(
          ...
          },
          {
            args: {
              ...
              metadata: {
                name: "Fox Federation" + i,
                symbol: "TFF" + i,
                uri: "https://arweave.net/o4lmnTk7pSqcnw8iI4yQKWOZchqBKIKPhYo7zTbbIAo",
              },
            },
          }
        );

        const signers = [adminHC.identity().signer as KeypairLike, mintKeypair];
        const op = new Operation(adminHC, [ix], signers);
        ...
        // pushing the resource to the resources array
        data.nonFungible.resources.push({
          label: labels[i],
          resource: resourceAddress,
          mint: mintKeypair.publicKey,
          tree: PublicKey.default,
    ...
    it.skip("Initialize Resource Tree", async () => {
      const resources = data.nonFungible.resources
        .map((e) => (e.tree.equals(PublicKey.default) ? e : null))
        .filter((e) => e) as Resource[];
    ...
    it.skip("Create Lookup Table", async () => {
      ...
      // setting up the lookup table if it exists
      if (data.nonFungible.lookupTableAddress) {
        lookupTable = (
          await adminHC.processedConnection.getAddressLookupTable(
            data.nonFungible.lookupTableAddress
          )
        ).value;
        return;
      }
      ...
      const extendLutInstruction = AddressLookupTableProgram.extendLookupTable({
        addresses: [
          ...data.nonFungible.resources.map((e) => e.resource),
          ...data.nonFungible.resources.map((e) => e.tree),
          adminHC.project().address,
          TOKEN_PROGRAM_ID,
          METADATA_PROGRAM_ID,
          SPL_NOOP_PROGRAM_ID,
      ...
      data.nonFungible.lookupTableAddress = lookupTableAddressPub;
      lookupTable = (
        await adminHC.processedConnection.getAddressLookupTable(
          data.nonFungible.lookupTableAddress
        )
    ...
    it.skip("Initilize Recipe", async () => {
      if (data.nonFungible.recipe.address) return;
      if (data.nonFungible.resources.length === 0)
        throw new Error("No Resources Found");
      ...
      const ix = createInitilizeRecipeInstruction(accounts, {
        args: {
          outputCharacteristics: new Map(),
          amounts: data.nonFungible.resources.map((e) => 100),
          xp: {
            label: "XP",
            increament: 100,
          },
      ...
      console.log(signature, "Initilize Recipe");
      data.nonFungible.recipe.address = recipeAddress;
    ...
    it.skip("Craft Recipe", async () => {
      ...
      if (data.nonFungible.resources.length === 0)
        throw new Error("No Resources Found");
      if (data.nonFungible.recipe.isCrafted) return;

      /// preparing the accounts for recipe
      const ixAccounts: CraftRecipeInstructionAccounts = {
        recipe: data.nonFungible.recipe.address,
        outputResource: PublicKey.default,
        inputResourceOne: PublicKey.default,
        inputResourceTwo: undefined,
        inputResourceFour: undefined,
      ...
      // setting the flag to true
      data.nonFungible.recipe.isCrafted = true;
